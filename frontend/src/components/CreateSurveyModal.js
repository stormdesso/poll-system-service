...
function CreateSurveyModal({ closeModal }) {
    ...
    async function fetchUser() {
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...formData,
            addressId: user.addressInfos[0].id
          });
  ...
  const handlePollValueChange = (index, value) => {
    const newPollValues = [...formData.pollValues];
    newPollValues[index] = value;
    setFormData({
      ...formData,
  ...
  const handleSubmit = async (e) => {
    ...
    const { title, addressId, startDate, endDate, cyclic, description, file, pollValues, scheduleType, multi } = formData;
    ...
    const payload = {
      ...
      scheduleType: cyclic === 'Да' ? scheduleType : 'NO_SCHEDULE'
    };
    console.log(payload);

    try {
      const response = await fetch('http://localhost:8080/api/v1/poll/create', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        console.log('Опрос создан успешно');
