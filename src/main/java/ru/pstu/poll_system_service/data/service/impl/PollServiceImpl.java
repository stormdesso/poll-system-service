...
public class PollServiceImpl implements PollService {
    ...
    private Poll getPoll(Long pollId) {
        var poll = pollRepository.findPollByIdEquals(pollId).orElseThrow(() -> {
            log.info("Опрос не существует или не доступен!");
            return new ResponseStatusException(HttpStatus.NOT_FOUND, "Опрос не существует или не доступен!");
        });
    ...
    public void vote(Long pollId, List<PollValueDto> pollValueDtoList) {
        ...
        if (pollRepository.userIsVoted(pollId, userId)) {
            log.info("Пользователь уже голосовал в опросе!");
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Пользователь уже голосовал в опросе!");
        }
        ...
        if (!pollValuesIds.containsAll(pollValueDtosIds)) {
            log.info("Указаны неверные варианты ответа!");
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Указаны неверные варианты ответа!");
        }

        var votes = userAnswerRepository.getNumberOfVotesByUser(userId, pollId);
        if (poll.getMaxNumberAnswersByUser() <= votes) {
            var message = "Максимальное число вариантов ответа для пользователя, " +
        ...
        if (pollValueDtosIds.stream().distinct().count() != pollValueDtoList.size()) {
            log.info("Нельзя проголосовать за один вариант ответа более 1 раза!");
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "Нельзя проголосовать за один вариант ответа более 1 раза!");
        }
    ...
    private void validatePoll(Long addressIdFromPoll, List<PollValueDto> pollValues, Date startDate, Date endDate) {
        ...
        if (!addressesIds.contains(addressIdFromPoll)) {
            log.info("Нет доступа к адресу!");
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Нет доступа к адресу!");
        }

        if ((pollValues.size() < 2)) {
            log.info("Минимальное число вариантов ответа: 2!");
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Минимальное число вариантов ответа: 2!");
        }

        if (startDate.after(endDate)) {
            log.info("Неверно указаны даты начала и завершения опроса!");
    ...
    public Poll update(PollDto pollDto) {
        ...
        Poll originalEntity = pollRepository.findById(pollDto.getId()).orElseThrow(() -> {
            log.info("Указан неверный идентификатор опроса!");
            return new ResponseStatusException(HttpStatus.NOT_FOUND, "Указан неверный идентификатор опроса!");
        });
        ...
        if (!(StatusEnum.valueOf(StatusEnum.class, originalEntity.getStatus()).equals(StatusEnum.proposed) ||
            ...
            log.info("Невозможно отредактировать опрос!");
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Невозможно отредактировать опрос!");
        }

        if (!(originalEntity.getCreatorUserId().equals(getCurrentUserIdFromContext()) ||
                roles.contains(admin.name()) || roles.contains(root.name()))) {
            log.info("Нет прав на редактирование опроса!");
            throw new AccessDeniedException("Нет прав на редактирование опроса!");
        }
